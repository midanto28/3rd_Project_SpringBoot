...
const ListComponent = () => {
  ...
  const {isFetching:isFetchingOne, data:dataOne, error:errorOne, isError:isErrorOne} = useQuery( 
    ['products/listone' , {page,size:24,refresh}],   //key값 page,size,refresh 요소가 변경될때마다 렌더링됨
    () => getListOne({page,size:24}), //page,size를 파라미터로해서 getList호출
...
const formatPrice = (price) => {
  ...
  return ( <>
        ...
        <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
          <h2 className="text-2xl font-bold tracking-tight text-gray-900">단일상품</h2>
